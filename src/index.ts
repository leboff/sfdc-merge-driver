...
import { MetadataItem, MetadataMerge } from './metadata/MetadataItem'
...
class SfdcMergetool extends Command {
  ...
  async run() {
    ...
    return new Promise((resolve, reject) => {
      let mergeResult = ChildProcess.spawn('git', ['merge-file', args.current , args.base, args.other], { cwd: process.cwd()})
      mergeResult.on('exit', (exitCode: number, status: string)=>{
        if(exitCode){
          this.log(`Found ${exitCode} conflicts`);
